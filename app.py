...
    # Ensure description is treated as string and handle potential NaNs
...
    found_skills = [skill for skill in skills if skill.lower() in description_lower]
    return found_skills

try:
    # Apply skill extraction on the filtered data, not the original df
    if 'job_description' in filtered_df.columns:
        filtered_df['skills'] = filtered_df['job_description'].apply(extract_skills)
         ...
         # Set top_skills to empty to prevent errors later
         top_skills = []


except KeyError as e:
    st.error(f"Error accessing 'job_description' column for skills extraction. Please check your data.  KeyError: {e}")
...
# Only proceed if filtered_df has 'skills' column and it's not empty after extraction
if 'skills' in filtered_df.columns and not filtered_df['skills'].empty:
    ...
        skills_df = pd.DataFrame(top_skills, columns=['Skill', 'Count'])
        # Ensure 'Skill' column is string type
    ...
            title_font_size=22,
            font=dict(family='Inter', size=12),
             # Not adding explicit axis titles here to stick to "no layout nothing"
        )
    ...
            "<div class='plot-container'><p style='color:#ffaaaa;'>No skills found in the job descriptions.</p></div>",
             unsafe_allow_html=True) # Ensure this uses unsafe_allow_html

else:
    st.markdown(
        "<div class='plot-container'><p style='color:#ffaaaa;'>No job description data available for skills analysis.</p></div>",
         unsafe_allow_html=True) # Ensure this uses unsafe_allow_html



# # --- Job Type Analysis --- (Still commented out as per your instruction)
